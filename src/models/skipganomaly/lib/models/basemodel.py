...
class BaseModel():
    ...
    def set_input(self, input:torch.Tensor, noise:bool=False):
        ...
        """
        with torch.no_grad():
            self.ids.resize_(input[0].size()).copy_(input[0])
    ...
    def save_anomaly_scores(self, scores, savedir = "train", fname="scores.pkl"):
        """Save ground thruth and anomaly scores for the current epoch.

        Args:
            scores: scores dictionary
        """
        anom_dir = os.path.join(self.opt.outf, self.opt.name, savedir, 'anom')
        if not os.path.exists(anom_dir): os.makedirs(anom_dir)
        filename = os.path.join(anom_dir, fname)
    ...
    def save_errors(self, epoch, total_steps, errors):
        """Save errors for the current epoch.

        Args:
            errors: errors dictionary
        """
        errors_dir = os.path.join(self.opt.outf, self.opt.name, 'train', 'errors')
        if not os.path.exists(errors_dir): os.makedirs(errors_dir)
        filename = os.path.join(errors_dir, "errors.pkl")
        if not os.path.exists(filename):
            f = open(filename, "wb")
        else:
            f = open(filename, 'ab')
    ...
    def residuals(self, fake, real):
        difference = np.subtract(real.permute(1,2,0).cpu().numpy(), fake.permute(1,2,0).cpu().numpy())

        if self.opt.nc == 3:
            total_difference = np.add(difference[:, :, 0], difference[:, :, 1])
            total_difference = np.add(total_difference, difference[:, :, 2])
            res_scaled = np.interp(total_difference, (total_difference.min(), total_difference.max()), (0, 1))
        elif self.opt.nc == 1:
            res_scaled = np.interp(difference, (difference.min(), difference.max()), (0, 1))
    ...
    def load_weights(self, epoch=None, is_best:bool=False, path=None):
        ...
        if path is None:
            ...
            path_g = os.path.join(path, fname_g)
            path_d = os.path.join(path, fname_d)
    ...
    def train_one_epoch(self):
        ...
        for data in tqdm(self.data.train, leave=False, total=len(self.data.train)):
            ...
            if self.total_steps % self.opt.print_freq == 0:
                ...
                self.save_errors(self.epoch, self.total_steps, errors)
                if self.opt.display:
    ...
    def train(self):
        ...
        for self.epoch in range(self.opt.iter, self.opt.niter):
            ...
            res, scores = self.test()
            recall, _, _, _ = recall_(scores)
            self.save_anomaly_scores(scores)
            if recall > best_auc:
                best_auc = recall
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            if self.opt.load_weights:
                ...
                path= os.path.join(self.opt.resume, "netG_best.pth")
            ...
            for i, data in enumerate(self.data.valid, 0):
                ...
                self.ids_total[i * self.opt.batchsize: i * self.opt.batchsize + error.size(0)] = self.ids.reshape(error.size(0), 2)
