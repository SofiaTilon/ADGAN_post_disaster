...
class Skipganomaly(BaseModel):
    ...
    def __init__(self, opt, data=None):
        ...
        if self.opt.resume != '':
            ...
            self.opt.iter = torch.load(os.path.join(self.opt.resume, 'netG_0.pth'))['epoch']
            self.netg.load_state_dict(torch.load(os.path.join(self.opt.resume, 'netG_0.pth'))['state_dict'])
            self.netd.load_state_dict(torch.load(os.path.join(self.opt.resume, 'netD_0.pth'))['state_dict'])
            print("\tDone.\n")
        ...
        self.input = torch.empty(size=(self.opt.batchsize, opt.nc, self.opt.isize, self.opt.isize), dtype=torch.float32, device=self.device) # 3-> opt.nc
        self.noise = torch.empty(size=(self.opt.batchsize, opt.nc, self.opt.isize, self.opt.isize), dtype=torch.float32, device=self.device) # 3-> opt.nc
        self.label = torch.empty(size=(self.opt.batchsize,), dtype=torch.float32, device=self.device)
        self.gt = torch.empty(size=(opt.batchsize,), dtype=torch.long, device=self.device)
        self.fixed_input = torch.empty(size=(self.opt.batchsize, opt.nc, self.opt.isize, self.opt.isize), dtype=torch.float32, device=self.device) # 3-> opt.nc
    ...
    def test(self, plot_hist=False):
        ...
        with torch.no_grad():
            ...
            if self.opt.load_weights:
                self.load_weights(is_best=True, path=self.opt.resume)
