...
class Visualizer():
    ...
    def save_current_images(self, epoch, reals, fakes, fixed):
        ...
        Args:
            ...
            fixed ([FloatTensor]): Fixed Fake Image
        """
        vutils.save_image(reals, '%s/reals.jpg' % self.img_dir, normalize=True)
        vutils.save_image(fakes, '%s/fakes.jpg' % self.img_dir, normalize=True)
        vutils.save_image(fixed, '%s/fixed_fakes_%03d.jpg' %(self.img_dir, epoch+1), normalize=True)
